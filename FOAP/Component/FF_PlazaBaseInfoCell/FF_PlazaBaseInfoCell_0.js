...
var FFPlazaBaseInfoCell = React.createClass({
    ...
    renderRow:function(data,sectionID, rowID, adjacentRowHighlighted) {
        var windowWidth =  Dimensions.get('window').width;
        var lineCount = (this.state.dataSource.getRowCount() + 1) / 2;
        return (
            <TouchableHighlight key={rowID}
                                onPress={() => this.tappAdvertise(data)}>
               <View style = {{width:windowWidth / lineCount, height:windowWidth * 146.0 / 640,alignItems:'center'}}>
                   <Image style = {{width:35, height:35,top:10}}
                          source={this.iconName(data['type'])}>
                   </Image>
                   <Text style = {{top:15,fontSize:10}}>
                       {data['name']}
                   </Text>
               </View>
            </TouchableHighlight>
    ...
    tappAdvertise:function(data) {
        ...
        //         par['dataDic'] = dic
        //         par['title'] = '排队取号'
        //         par['moduleName'] = 'AwesomeProject'
        //
        //         par['urlString'] = 'http://localhost:8081/index.ios.bundle?platform=ios&dev=true'
        //         var schme  = require('react-native').NativeModules.FFReactManager
